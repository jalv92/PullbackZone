...
- **Pullback spans ≥2 closed bars (spec amendment, Javier-approved 2026-08-05):** the trigger hunt arms no earlier than the close of the second bar after the leg-extreme bar. Structural rule (no new dial). `stop_buffer_atr30` re-frozen under it in one pre-registered pass; both mirror sides implement the identical bar-count rule.
