...
- **Fast-pullback window (Amendment 2, Javier-approved 2026-08-05, supersedes the ≥2-bar span reading):** the hunt arms ONLY at the close of bar `ext_i + 2`, with the depth known through that bar — one-bar-wick delivery allowed BY DESIGN, later-than-2-bars delivery excluded. New extreme resets the window. `stop_buffer_atr30` re-frozen once per amendment; both mirror sides implement the identical `i == ext_i + 2` rule. The single-bar-noise suspicion stays pre-registered in validation.md.
- **Flat-to-flat, globally (pinned after Task 3's engine-consistency check):** while any fill's position is unresolved (stop/target/flatten not yet printed) or an entry order is working, NO new entry may be generated by ANY leg — cross-leg included. Without this, episodes() logs fills the engine silently drops (measured: 2/62 on 40 sessions) and NT8 would REVERSE the position on an opposite-direction signal. The NT8 side implements it as: no entry submission while `Position.MarketPosition != Flat` or the entry order is live.
...
11. **Early entry cancels (Task 6, NT8-only).** PropSim resolves an entry's fate inside the trigger bar's own iteration, so its order always works the full TTL (`busy = t_ttl`). NT8 lives forward in time and cancels a working entry when its leg DIES or when a new leg extreme supersedes the pullback it was priced from, on top of the TTL / flatten cancels. The second case has a narrow reachable window (a new extreme within `entry_offset_ticks − 1` ticks above the old one resets the hunt without filling — one tick wide at the default offset), and it is NOT conservative: NT8, having cancelled early, can re-arm at the new `ext+2` and take an entry INSIDE the TTL window PropSim had blocked with `busy = t_ttl` — an NT8-EXTRA entry is this delta's signature, not a pattern bug. The leg-death case IS conservative (NT8 takes fewer entries) and `ttl`-reachable deaths include `reentry`, `timeout`, and plausibly `destination`. The NT8 corpus carries `reason` on every `expired` row (`ttl` / `leg_died` / `hunt_reset` / `lockout` / `rejected` / `not_submitted` / `position_closed`), so Task 7 MEASURES this instead of assuming: a PropSim `filled` matched to an NT8 `expired` with a non-`ttl` reason is this delta, not a pattern bug.
12. Manual exits and session flattens grant NO attempt 2 (matches PropSim's `_resolve_exit` "unresolved" handling; only a real stop-out re-arms). Javier closing a trade by hand in Playback is expected — the gate must not read the missing attempt 2 as a divergence.
13. A stop-market entry can fill through its price on a gap; if the fill lands beyond `target_r × risk`, the frozen target limit goes in already marketable and round-trips instantly — an `exit/target` row at a price far from `target_px` is a FILL-SLIPPAGE artifact, not a pattern failure. Task 7's exit-price check classifies these separately.
14. End-of-tape terminal row: PropSim emits one final `leg_died`/`no_attempt_left` for a leg alive at the slice end; NT8 has no equivalent moment. One PropSim-only row per run is expected.

**NT8-only corpus fields (Task 6).** Additive, ignored by a joiner that keys on the shared vocabulary: an `exit` row per closed position (`reason` = stop/target/flatten/manual + `exit_px`) — the only source for V1's exit-price check — plus `fill_px` on `filled` rows and `reason` on `expired` rows. Prices in the shared fields stay UNROUNDED (what PropSim computes); the .cs rounds to the tick grid only at order submission, a ≤ half-tick difference inside the gate's 1-tick tolerance.
