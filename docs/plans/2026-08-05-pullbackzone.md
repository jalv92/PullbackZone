...
11. **Early entry cancels (Task 6, NT8-only).** PropSim resolves an entry's fate inside the trigger bar's own iteration, so its order always works the full TTL (`busy = t_ttl`). NT8 lives forward in time and cancels a working entry when its leg DIES or when a new leg extreme supersedes the pullback it was priced from, on top of the TTL / flatten cancels. The second case is all but unreachable (a new extreme lies beyond the entry stop, which therefore filled on the way); the first is bounded by the 3-minute TTL and realistically only "reentry" deaths reach it. Direction is conservative (NT8 takes fewer entries). The NT8 corpus carries `reason` on every `expired` row (`ttl` / `leg_died` / `hunt_reset` / `lockout` / `rejected`), so Task 7 MEASURES this instead of assuming: a PropSim `filled` matched to an NT8 `expired` with a non-`ttl` reason is this delta, not a pattern bug.
